...
function HospitalAnimation() {
  ...
  const MonitorCard = (
    <div style={{ width:isMobile?"100%":260, background:LIGHT, border:`1.5px solid ${isAlarm?"#e8b4b0":MID}`, borderRadius:12, padding:20, transition:"border-color 1s, box-shadow 1s", boxShadow:isAlarm?"0 0 30px rgba(192,57,43,0.08)":"0 1px 8px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ background:SOFT, borderRadius:6, padding:"10px 12px", marginBottom:14, height:72, overflow:"hidden", border:`1px solid ${MID}` }}>
        <svg width="200" height="52" viewBox="0 0 200 52" style={{ display:"block", width:"100%" }}>
  ...
  const ResultCard = (
    <div style={{ width:isMobile?"100%":260, background:LIGHT, border:`1.5px solid ${isResult?"#a8d5b5":MID}`, borderRadius:12, padding:20, opacity:isResult?1:0.2, transform:isResult?"translateY(0)":"translateY(10px)", transition:"opacity 1.2s, transform 1.2s, border-color 1.2s", boxShadow:isResult?"0 1px 8px rgba(0,0,0,0.06)":"none" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
      ...
      <div>
        <div style={{ textAlign:"center", marginBottom:20 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"6px 16px", borderRadius:100, background:isAlarm?"#fdf0ef":isAnalyze?"#edf7f1":isResult?"#edf7f1":SOFT, border:`1px solid ${isAlarm?"#e8b4b0":isAnalyze?"#a8d5b5":isResult?"#7ec8a0":MID}`, transition:"all 1s" }}>
            ...
            <span style={{ fontSize:11, color:phaseLabelColor, transition:"color 1s" }}>{phaseLabel}</span>
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {MonitorCard}
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"8px 0" }}>
            <div style={{ height:1, flex:1, background:MID }} />
            <div style={{ background:isAnalyze||isResult?"#edf7f1":SOFT, border:`1px solid ${isAnalyze||isResult?"#a8d5b5":MID}`, borderRadius:8, padding:"8px 16px", fontSize:11, color:isAnalyze?"#27ae60":isResult?"#1e8449":"#95a5a6", transition:"all 1s" }}>
              {phase===0?"Watching...":phase===1?"Alarm detected":phase===2?"Checking...":"✓ Done"}
            </div>
            <div style={{ height:1, flex:1, background:MID }} />
          </div>
          {ResultCard}
...
function IndustrySection() {
  const isMobile = useIsMobile()
  const companies = [
    { name:"Traditional bedside monitors", what:"The monitors in every ICU around the world are engineering marvels. They measure blood oxygen, heart rhythm, and breathing with remarkable precision.", gap:"They were designed to compare a reading against a fixed threshold. If blood oxygen drops below 90%, the alarm fires every time - even if the sensor just slipped off the finger.", tag:"Hardware" },
    { name:"Threshold-based alerting systems", what:"Some newer systems let hospitals customize alert thresholds, so doctors can set alarm levels based on a patient's condition instead of relying on generic defaults.", gap:"Customized thresholds still don't know whether a reading is trustworthy. A low blood oxygen alarm fires whether the patient is deteriorating or whether someone bumped the sensor.", tag:"Alerting" },
    { name:"Single-channel monitoring devices", what:"Modern devices measure individual signals with high accuracy. Each sensor is calibrated, validated, and reliable on its own.", gap:"They analyze each signal in isolation. No device really asks: if blood oxygen looks bad but heart rhythm is normal, should we trust that oxygen reading?", tag:"Devices" },
    { name:"Remote alarm management platforms", what:"Cloud-connected platforms let clinical staff view alarms from anywhere - a nurse can silence or escalate an alert from a phone instead of walking to the bedside.", gap:"The nurse is still the final decision maker for every single alarm. There's no system that evaluates whether the signal is trustworthy before it reaches them.", tag:"Software" },
  ]

  return (
    <section style={{ borderTop:`1px solid ${MID}`, background:LIGHT }}>
      <div style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"48px 20px":"80px 40px" }}>
        <Reveal>
        ...
        <Reveal delay={0.1}>
          <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:16, marginBottom:32 }}>
            {companies.map((c,i) => (
              <div key={i} style={{ background:SOFT, border:`1px solid ${MID}`, borderRadius:12, padding:isMobile?20:28 }}>
                ...
                <div style={{ paddingTop:14, borderTop:`1px solid ${MID}` }}>
...
function MLSection() {
  ...
  const experiments = [
    {
      ...
      results:[{name:"EfficientNet + Neural Classifier", auc:0.641, best:true},{name:"EfficientNet + Logistic Regression", auc:0.587, best:false},{name:"ResNet18 + SVM", auc:0.542, best:false}],
      finding:"EfficientNet won. It correctly identified real versus false alarms 64% of the time, outperforming every other approach. The visual heat map method captures signal patterns that are invisible when looking at raw numbers alone.",
    ...
    },
    {
      ...
      results:[{name:"SVM (best settings)", auc:0.539, best:true},{name:"XGBoost (tuned)", auc:0.517, best:false},{name:"Gradient Boosting", auc:0.465, best:false}],
      finding:"This approach underperformed the image-based method. The hand-defined measurements were not as informative as the visual patterns the AI discovered on its own.",
    ...
    },
    {
      ...
      detail:"Hospitals deal with four main alarm types: irregular heartbeat, stopped heart, too fast, and too slow. Each one looks different on a monitor. A beat detector was built to identify individual heartbeats, measure their shape and timing, then a separate model was trained for each alarm type.",
      models:["XGBoost per alarm type","Pan-Tompkins Beat Detector"],
      results:[{name:"Rapid heartbeat model", auc:0.612, best:true},{name:"Irregular heartbeat model", auc:0.528, best:false},{name:"Stopped heart model", auc:0.478, best:false}],
  ...
  return (
    <section style={{ borderTop:`1px solid ${MID}`, background:SOFT }} id="ml-section">
      <div style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"48px 20px":"80px 40px" }}>
        <Reveal>
              ...
              <div style={{ background:LIGHT, border:`1px solid ${MID}`, borderRadius:12, padding:isMobile?16:32 }}>
                <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
                  <div>
                    ...
                    <p style={{ fontSize:13, color:"#7f8c8d", lineHeight:1.8, marginBottom:14 }}>{exp.detail}</p>
                  ...
                  <div style={{ borderTop:isMobile?`1px solid ${MID}`:"none", paddingTop:isMobile?16:0 }}>
                    ...
                    <div style={{ padding:"10px 14px", background:SOFT, borderRadius:6, border:`1px solid ${MID}` }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <span style={{ fontSize:11, color:"#95a5a6" }}>Random guessing</span>
    ...
    <div style={{ minHeight:"100vh", fontFamily:"'DM Sans','Helvetica Neue',sans-serif", background:SOFT }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .step{display:flex;gap:16px;padding:20px 0;border-bottom:1px solid ${MID};}
      ...
      <div style={{ position:"relative" }}>
        ...
        <section style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"110px 20px 48px":"140px 40px 60px", textAlign:"center" }}>
          ...
          <h1 className="fi d2" style={{ fontSize:isMobile?"clamp(28px,8vw,40px)":"clamp(34px,5.5vw,58px)", fontWeight:300, color:CHARCOAL, lineHeight:1.2, letterSpacing:isMobile?"-1px":"-2px", marginBottom:18 }}>
        ...
        {/* Alert strip */}
        <section style={{ background:CHARCOAL, padding:isMobile?"20px":"24px 40px", textAlign:"center" }}>
          <p style={{ fontSize:isMobile?13:14, color:"#95a5a6", maxWidth:760, margin:"0 auto", lineHeight:1.8 }}>
        ...
        <section style={{ borderTop:`1px solid ${MID}`, borderBottom:`1px solid ${MID}`, background:LIGHT }}>
          <Reveal>
            <div style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"0 20px":"0 40px", display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4, 1fr)" }}>
              {[
                ...
                <div key={i} style={{ padding:isMobile?"24px 0":"36px 0", ...(isMobile?{borderBottom:i<2?`1px solid ${MID}`:"none",...(i%2===1?{paddingLeft:20,borderLeft:`1px solid ${MID}`}:{})}:{...(i>0?{paddingLeft:40,borderLeft:`1px solid ${MID}`}:{}),paddingRight:40}) }}>
                  ...
                  <div style={{ fontSize:isMobile?11:13, color:"#95a5a6", marginTop:6 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </Reveal>
        ...
        {/* Problem */}
        <section style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"48px 20px":"80px 40px" }}>
          <Reveal>
            <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:isMobile?32:60, alignItems:"start" }}>
              ...
              <div className="card" style={{ padding:isMobile?20:28 }}>
                <p style={{ fontSize:11, color:"#95a5a6", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:14 }}>The question no monitor currently asks</p>
                {[
          ...
          <div style={{ maxWidth:960, margin:"0 auto", padding:isMobile?"48px 20px":"80px 40px" }}>
            <Reveal>
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:isMobile?32:60 }}>
                <div>
                  <p style={{ fontSize:11, color:"#95a5a6", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:20 }}>How it works</p>
                  <div>
                    {[
                      {n:"01", title:"It reads the raw signal", body:"SigmaMedStat analyzes the raw electrical signals from the sensor - not just the final reading the monitor displays."},
                ...
                <div>
                  <p style={{ fontSize:11, color:"#95a5a6", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:16, marginTop:isMobile?8:0 }}>What it detects</p>
                  {[
                    {label:"Sensor fell off or slipped", sub:"The most common causes of false alarms are motion, sweat, and repositioning."},
                    {label:"Electrical interference", sub:"Nearby equipment can interfere with a reading without touching the patient."},
                    {label:"Signal dropped out completely", sub:"Total loss of data - often a cable issue, not a patient issue."},
                    {label:"Gradual signal drift", sub:"What looks normal at 8am may trigger an alarm by 4pm after a long session."},
                    ...
                    <div key={i} style={{ padding:"12px 16px", background:SOFT, border:`1px solid ${MID}`, borderRadius:8, marginBottom:4 }}>
