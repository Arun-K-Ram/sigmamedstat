...
const SCENARIOS: Record<string, any> = {
  v100s: {
    ...
    confidence: 58,
    what_sigmamedstat_found: "TThe model analyzed 60 seconds of raw heart signal data, converted it into a visual pattern, and extracted 1,280 features. It identified patterns that resembled a true cardiac event, but in this case it was incorrect.",
    ...
    honest_note: true,
    why_wrong: "This type of irregular heartbeat (ventricular flutter) is the hardest alarm for the model to classify-per-alarm analysis shows the ventricular classifier performs worse than tachycardia. A model trained on this patient’s individual baseline would likely have caught it. Without signal-level intelligence, this alarm would have been one of 350+ that day. A nurse would have responded, but the 10 minutes spent on a false alarm is 10 minutes taken away from a patient who might actually need attention."
  },
  v101l: {
    id: "v101l",
    label: "Irregular heartbeat alarm",
    ...
    honest_note: false,
    why_wrong: "",
    without: "Without signal-level intelligence, this alarm looks identical to hundreds of others in a shift. The difference between this and a false alarm isn’t obvious to the human eye, but it becomes clear in the signal data."
  },
  a103l: {
    id: "a103l",
    ...
    ground_truth: true,
    ground_truth_plain: "The monitor detected a flatline : no detectable heartbeat-and triggered its highest-priority alarm.",
    ...
    verdict_correct: true,
    confidence: 64,
    what_sigmamedstat_found: "The model identified patterns in the signal consistent with genuine cardiac arrest. The absence of a normal heartbeat rhythm across the full signal window was a strong indicator.",
  ...
  a104s: {
    ...
    action: "The model flagged it as “act now” with high confidence, but the patient was completely fine. This is the kind of error that can quickly erode trust in automated systems.",
    honest_note: true,
    why_wrong: "High-confidence mistakes are the most dangerous kind. The signal had features that genuinely resembled a cardiac arrest pattern, which is why even experienced clinicians find this alarm type hard to dismiss. Patient-specific baselines could make a major difference here.",
    without: "Even without the model, a nurse would have responded to this alarm-it’s a cardiac arrest alert. The cost is 5–10 minutes of emergency response for a patient who was actually fine. Multiply that by the number of false cardiac arrest alarms per week."
  },
  t107l: {
    id: "t107l",
    label: "Rapid heart rate alarm",
    bed: "Bed 2 · Step-Down Unit",
    situation: "A patient’s heart rate alarm fired for being too fast. A rapid heart rate can indicate many things-fever, pain, anxiety, dehydration, or a genuine cardiac issue. Signal data can help distinguish between these causes.",
    ...
    honest_note: false,
    why_wrong: "",
    without: "Rapid heart rate alarms are extremely common-many are triggered by movement, coughing, or temporary agitation. A nurse who has seen 50 tachycardia alarms in a day is less likely to treat each one as urgent. SigmaMedStat signal analysis helps restore the appropriate level of urgency to these cases."
  },
  b124s: {
    id: "b124s",
    label: "Slow heart rate alarm",
    bed: "Bed 5 · Cardiac Unit",
    situation: "A patient’s heart rate dropped below the threshold and triggered a slow heart rate alarm. The patient’s other readings appear completely normal, which is unusual - genuinely slow heart rate rarely occurs in isolation.",
    ...
    verdict_correct: true,
    confidence: 55,
    what_sigmamedstat_found: "The model found that the signal was more consistent with a sensor reading error than a true slow heart rate. Confidence was modest at 55% - a close call - but it leaned toward a false alarm, which turned out to be correct.",
    ...
    honest_note: false,
    why_wrong: "",
    without: "A slow heart rate alarm on a cardiac unit requires a response. If it’s a false alarm-as it was here - the nurse spends time assessing a patient who doesn’t need it, documents the response, and moves on. When this happens dozens of times a day, it adds up to hours of lost clinical time per shift."
  }
}
      ...
      <div style={{ maxWidth:1060, margin:"0 auto", padding:"90px 40px 60px" }}>
        ...
        <div style={{ marginBottom:36, maxWidth:680 }}>
          ...
          <p style={{ fontSize:14, color:"#7f8c8d", lineHeight:1.8 }}>
            Each scenario below comes from real patient alarm recordings in a published hospital dataset. Select any one - the model will analyze the heart signal and determine whether the alarm is worth acting on. We show where it gets it right, and where it gets it wrong.
          </p>
        </div>
